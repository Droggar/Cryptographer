...
def show_algorithms():
    button_algorithms.place_forget()
    button_change_language.place_forget()
...
def show_edca():
    root.destroy()
    current_directory = os.path.dirname(__file__)
    file_path = os.path.join(current_directory, 'ECDSA.py')
    exec(open(file_path, encoding="utf-8").read(), globals())
...
def show_ecdh():
    root.destroy()
    current_directory = os.path.dirname(__file__)
    file_path = os.path.join(current_directory, 'ECDH.py')
    exec(open(file_path, encoding="utf-8").read(), globals())
...
button_algorithms = tk.Button(root, text="Algorytmy", command=show_algorithms, width=20, height=2)
button_change_language = tk.Button(root, text="Zmiana języka", command=change_language, width=20, height=2)
button_exit = tk.Button(root, text="Wyjście", command=exit_application, width=20, height=2)
